...
},
{
  ...
  "number": "1.2.1",
  "title": "Conditional.",
  "body": " Conditional  conditional  Given a propositions and ,   the conditional statement  is the proposition  \"If then \"  Proposition is called the antecedent and is the consequent .  The conditional statement is true exactly when is false or is true.   "
},
{
...
},
{
  ...
  "number": "1.2.1",
  "title": "Biconditional.",
  "body": " Biconditional  biconditional  Given a propositions and ,   the biconditional statement  is the proposition  \" if and only if \"  The biconditional statement is true exactly when and have the same truth value.   "
},
{
...
},
{
  ...
  "number": "1.5.1",
  "title": "",
  "body": "   For every Indiana resident older than 18, can vote     Some functions defined at 0 are not differentiable at zero     Some students are math majors and some students are business majors     "
},
{
  "id": "sec-1-4",
  "level": "1",
...
},
{
  ...
  "number": "2.2.3",
  "title": "",
  "body": " argument  For this example lets set a few propositions:       Next consider the list of premises.                        When can then make the following arguments:   To start we will only consider the first premise.            Since we have assumed that it is not sunny and colder than yesterday we can conclude it is not sunny outside, using conjunction.    Now Let's use the conclusion from (A), with the next premise.               That is if we assume that it is not sunny and that we will go hiking only if it is sunny, then we can conclude that we do not go hiking, using Modus Tollens.    Now using the conclusion from (B) and our next premises we see               Since we have assumed that we are not hiking and that if we don't hike then we will go on a bike ride we can thus conclude that we will go on a bike ride by Modus Ponens.    Now using the conclusion of (C) and our next premise.               Hence since we assumed that we are going on a bike ride and if we go on a bike ride we will be back home by dark we can conclude that we will be back by dark again by Modus Ponens.     We have just done our first proof, and shown if all the statements are assumed then we can conclude that we will be home before dark!  "
},
{
  "id": "sec-3-3-fallacy",
  "level": "1",
...
},
{
  ...
  "number": "3.4.1",
  "title": "2.",
  "body": " 2   Prove: For all integers and if and are both odd then is odd.  Select two random integers and . Assume that is even and is odd. By definition of even we can find an integer so that . Also, by the definition of odd we can find an integer so that . Now we calculate:   Now since is an integer, we see that is odd.  To summarize, since we chose arbitrary integers and which we assumed were even and odd, respectively, by direct proof we proved that for all integers and , if is even and is odd, then is odd.   "
},
{
...
},
{
  ...
  "number": "5.2.15",
  "title": "",
  "body": " For this example consider the universe of discourse as the integers, , and consider the set of all even numbers, that is the set Then the compliment of is all the integers that are not even, or in lieu of we have that   "
},
{
...
},
{
  ...
  "number": "5.5.2",
  "title": "",
  "body": " Consider the set:   To build the power set, it is customary to begin with the empty set, , which we know must be a member of the power set from we know that . Hence,   Next, we will of course need all of the singletons , that is all the sets containing a single element from     Finally, since for any set , as any element of is surely an element of , we have   thus the power set is:   "
},
{
